import unittest
from flight_bridge.core import (CrossRepoWriteBlocked, PRODUCTION_GATE_INPUTS, classify_provider_response, dedupe_decision, evaluate_offer,
    production_gate,
...
def offer(**changes):
    ...
    row={"outbound":{"origin":"GRU","destination":"LIS","duration_minutes":600,"segments":[segment]},"inbound":{"origin":"LIS","destination":"GRU","duration_minutes":600,"segments":[inbound]},"cabin":"economy","requires_self_transfer":False,"protected_self_transfer":False,"airport_change":False,"separate_tickets":False,"multiple_booking_required":False,"price":{"amount":4499.99,"currency":"BRL","status":"verified"}}
...
class CoreTests(unittest.TestCase):
    ...
    def test_missing_journey_protection_is_fail_closed(self):
        row=offer(); row.pop('protected_self_transfer'); self.assertEqual('NON_VALIDATABLE',evaluate_offer(row)['ELIGIBILITY_STATE'])
    ...
    def test_production_gate_requires_every_input_true(self):
        all_true={name:True for name in PRODUCTION_GATE_INPUTS}; self.assertTrue(production_gate(all_true)['PRODUCTION_GATE'])
        all_true['HUMAN_AUDIT_PASS']=None; result=production_gate(all_true); self.assertFalse(result['PRODUCTION_GATE']); self.assertEqual('PRE_PRODUCTION',result['STATE']); self.assertIn('HUMAN_AUDIT_PASS',result['UNKNOWN_GATES'])
